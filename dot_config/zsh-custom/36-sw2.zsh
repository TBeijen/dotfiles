...
sw2() {
  ...
  if [[ $# -eq 0 ]]; then
    ...
    # Apply mise's cleanup synchronously (same reason as activation path)
  ...
  # Apply mise env diff synchronously so AWS_PROFILE and other overlay vars
  # are set BEFORE the prompt redraws. Otherwise oh-my-posh / starship
  # renders one prompt without them (precmd hook order quirk).
  if command -v mise &>/dev/null; then
    eval "$(mise hook-env -s zsh 2>/dev/null)"
  fi
...
# List immediate subdirs of $1 that contain a mise.toml (= valid sw2 workspaces).
# For first arg (client) we don't filter — clients are just parent dirs.
_sw2_list_dirs() {
  local root="$1" must_have_mise="${2:-0}" d name
  [[ -d "$root" ]] || return 0
  for d in "$root"/*(N); do
    name="$(_sw2_basename "$d")"
    [[ "$name" == .* ]] && continue
...
# Parse role names from mise.<role>.toml files in the workspace dir.
_sw2_list_roles() {
  local ws="$1" f name role
  [[ -d "$ws" ]] || return 0
  for f in "$ws"/mise.*.toml(N); do
    name="$(_sw2_basename "$f")"
    # Strip leading 'mise.' and trailing '.toml'; skip the base mise.toml.
    role="${name#mise.}"
    role="${role%.toml}"
    [[ "$role" == "toml" ]] && continue
    print -r -- "$role"
...
_sw2_list_clusters() {
  local ws="$1" kube_dir="${1}/.kube" f name
  [[ -d "$kube_dir" ]] || return 0
...
if [[ -n ${ZSH_VERSION-} ]]; then
  ...
  if ! typeset -f _sw2 >/dev/null; then
    _sw2() {
      local -a suggestions
      ...
      case $state in
        client)
          suggestions=($(_sw2_list_dirs "$WS_ROOT"))
          _describe -t clients 'clients' suggestions && return
          ;;
        env)
          local client=${words[2]}
          # envs must have a mise.toml to be sw2-valid (skips legacy -ro/-ad dirs)
          suggestions=($(_sw2_list_dirs "$WS_ROOT/$client" 1))
          _describe -t envs 'envs' suggestions && return
          ;;
        role)
          local client=${words[2]} env=${words[3]}
          suggestions=($(_sw2_list_roles "$WS_ROOT/$client/$env"))
          _describe -t roles 'roles' suggestions && return
          ;;
        cluster)
          local client=${words[2]} env=${words[3]}
          suggestions=($(_sw2_list_clusters "$WS_ROOT/$client/$env"))
